...
def save_one_json(predn, jdict, path, class_map, im):
    # Save one JSON result {"image_id": 42, "category_id": 18, "bbox": [258.15, 41.29, 348.26, 243.78], "score": 0.236}
    image_id = int(path.stem) if path.stem.isnumeric() else path.stem
    box = xyxy2xywh(predn[:, :4])  # xywh
    box[:, :2] -= box[:, 2:] / 2  # xy center to top-left corner
    # box[:,0] = box[:,0]/im.shape[3]
    # box[:,1] = box[:,1]/im.shape[2]
    # box[:,2] = box[:,2]/im.shape[3]
    # box[:,3] = box[:,3]/im.shape[2]
...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            if save_json:
                save_one_json(predn, jdict, path, class_map, im)  # append to COCO-JSON dictionary
...
if __name__ == "__main__":
    opt = parse_opt()
    opt.imgsz = 416# 模型输入416*416 
    opt.conf_thres = 0.01# NMS置信度阈值
    opt.iou_thres = 0.5# NMS IOU 阈值
    opt.batch_size = 1


    opt.data = "./yolov3/data/pedestrain.yaml"
    # 指定测ap的数据集
    
    opt.weights = "./yolov3/runs/train/exp10/weights/best.pt"
    # 指定模型
    main(opt)
